...
/// workers, each on the *same* agent connection its dead counterpart used (drainer-1/mysql-agent-1,
/// drainer-2/mysql-agent-2, drainer-3/mysql-agent-3) -- AssignedLostBucketsRunner reassigns a dead
...
/// </summary>
public sealed class MySqlDistPhase2Emulator(ScenarioGlobalEnvironment global, ScenarioRunner runner)
    : BasePhaseEmulator<MySqlDistPhases>(global, runner)
{
    private const string ClusterId = "mysql-drain-load";
    private static readonly string[] AgentConnectionNames = ["mysql-agent-1", "mysql-agent-2", "mysql-agent-3"];
    ...
    public override MySqlDistPhases Phase() => MySqlDistPhases.Phase2;
