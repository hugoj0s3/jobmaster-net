...
/// connection (nats-agent-1/2/3), pooled alongside drainer-1/2/3 which keep running in the background
    ...
    : BasePhaseEmulator<PostgresNatsPhases>(global, runner)
{
    ...
    // turns), not a regression. Inherited unchanged from DrainModeTest.PostgresDist -- this scenario
    // hasn't been run yet to confirm NATS-specific timing (see scenario.md).
