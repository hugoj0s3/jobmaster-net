...
/// workers, each on the *same* agent connection its dead counterpart used (drainer-1/pg-agent-1,
/// drainer-2/pg-agent-2, drainer-3/pg-agent-3) -- AssignedLostBucketsRunner reassigns a dead
...
/// </summary>
public sealed class PostgresDistPhase2Emulator(ScenarioGlobalEnvironment global, ScenarioRunner runner)
    : BasePhaseEmulator<PostgresDistPhases>(global, runner)
{
    private const string ClusterId = "postgres-drain-load";
    private static readonly string[] AgentConnectionNames = ["pg-agent-1", "pg-agent-2", "pg-agent-3"];
    ...
    public override PostgresDistPhases Phase() => PostgresDistPhases.Phase2;
