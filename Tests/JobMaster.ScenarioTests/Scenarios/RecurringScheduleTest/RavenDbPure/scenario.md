...
**registration mode** (static, defined at app startup vs. dynamic, created at runtime via HTTP) x
**compiler** (`TimeSpanInterval` vs `NaturalCron`), plus two extra registration-path checks — a
static schedule registered purely via `[TimeSpanIntervalSchedule]` attribute auto-registration (no
profile), and a dynamic schedule created via
`IJobMasterScheduler.Advanced.RecurringAsync<TDefinition>()` (the `JobDefinitionConfigAttribute`
path) instead of the classic handler-typed overload. Unlike `Tests/JobMaster.IntegrationTests`'s
equivalent `RecurringScheduleTest` (seconds-level intervals, in-process handler, single process),
this scenario uses a realistic 6-minute interval and asserts through Redis + the JobMaster API
against a real containerized app, matching the `ScheduleTest` suite's architecture.
...
1. Creates 3 dynamic recurring schedules: `TimeSpanInterval`/`"00:06:00"` and
   `NaturalCron`/`"every 6 minutes"` via `POST /recurring-schedule/tick`, plus a third
   `TimeSpanInterval` schedule via `IJobMasterScheduler.Advanced.RecurringAsync<TDefinition>()`
   against `POST /recurring-schedule/advanced-tick` (proving the `JobDefinitionConfigAttribute`
   path fires/replans, not just the classic handler-typed overload) — each with a fresh
   `TestIdentifier`. The app also registers 3 *static* schedules at startup:
   `StaticTimeSpanIntervalProfile`/`StaticNaturalCronProfile` (fixed `TestIdentifier`s
   `"static-timespan-interval"`/`"static-natural-cron"`), plus `AttributeStaticTickHandler`,
   registered purely via `[TimeSpanIntervalSchedule]` with no profile at all (`TestIdentifier`
   `"attribute-static-tick"`, proving `RecurringScheduleAttribute` auto-registration end to end) —
   all duplicated as literals on this side since they're defined in a different process/project.
2. Asserts zero executions for all 6 within 20 seconds of creation/startup — catches an accidental
   "fires immediately" bug well before the real wait.
3. Waits (concurrently, not sequentially — ~17 minute budget) for each of the 6 to reach 2
   executions.
...
5. Cross-checks the 3 dynamic schedules against `GET /{clusterId}/recurring-schedules` —
   `ExpressionTypeId`/`Expression`/`JobDefinitionId` match (the advanced schedule's
   `JobDefinitionId` is `"RecurringApp.AdvancedTick"`, distinct from the other two's
   `"RecurringApp.Tick"`).
6. Cancels the 3 dynamic schedules, waits 90 seconds (`CancelJobsFromRecurScheduleInactiveOrCanceledRunner`
   ticks every 30s), then asserts no more than one additional execution occurred per schedule after
   cancellation (an in-flight job at cancellation time isn't retroactively killed, only future
   occurrences are prevented). Static schedules are left running — they're simply torn down with
   the container.
