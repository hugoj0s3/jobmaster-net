...
/// <summary>
/// Replaces the old JobMaster.IntegrationTests RunExecutionTest: schedules ~1000 jobs across all 3
/// clusters, mixing immediate and 5-minute-delayed jobs across several handler types. Every
/// cluster's TransientThreshold is 2 minutes (see Phase1/*.json) -- well under the 5-minute delay --
/// so this also proves a delayed job is not dispatched the moment it's threshold-eligible early, and
/// only executes once its actual due time arrives. The plan is built up front as a List&lt;JobsQty&gt;
/// and that same plan drives every assertion afterward, against both the Redis tracker and the
/// JobMaster API.
/// </summary>
public sealed class PostgresPurePhase1Emulator(ScenarioGlobalEnvironment global, ScenarioRunner runner)
    : BasePhaseEmulator<PostgresPurePhases>(global, runner)
{
    ...
    {
        var clusterIds = Enum.GetValues<PostgresPureClusters>()
    ...
    private static List<JobsQty> BuildPlan(IReadOnlyList<string> clusterIds)
    {
        ...
        // 333/cluster x 3 clusters = 999 -- effectively the ~1000 jobs asked for.
        return plan;
    }

    private async Task AssertApiAsync(IReadOnlyList<string> clusterIds, IReadOnlyList<ScheduledBatch> batches)
    {
        var api = Runner.Api ?? throw new InvalidOperationException("PostgresPure scenario has no api container configured.");
