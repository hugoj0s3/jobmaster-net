...
`TransientThreshold` is not dispatched early. Pure RavenDB (no other repo type involved).

## Topology

- `ravendb-standalone` — one `TargetTestScheduleApp` container, standalone cluster
  (`Standalone: true`, master + worker in one process), database `RavendbStandalone`.
- `ravendb-dist-one`, `ravendb-dist-two` — two `TargetTestScheduleApp` containers, each a
  *distributed* cluster: a separate master DB (`RavendbDistCluster`, shared by both) plus three
  agent connections (`raven-agent-1/2/3`, backed by
  `RavenAgent1`/`RavenAgent2`/`RavenAgent3`, also shared by both), each with its own worker. This
  is the first scenario in this suite where RavenDB is used as an agent connection, not just a
  cluster's master repository — already supported by `JobMaster.RavenDb`'s agent-side dispatcher
  repositories and covered in isolation by `Tests/JobMaster.IntegrationTests/RepoConformance/RavenDb/`.
...
- All databases are shared across clusters, isolated only by `ClusterId`/agent-connection name —
  same pattern as every other scenario in this suite.
- No `ConnectionOptions` tuning (e.g. `pooledConnectionLifetimeMs`) is set here — plain connection
  strings, matching every other RavenDB scenario variant in this suite. That binder behavior is
  already covered by unit tests (`Tests/JobMaster.UnitTests/Ioc/RavenDbConnectionOptionsTests.cs`),
  so scenario configs don't need to exercise it too.

## What Phase1 does

`RavenDbPureTests.RunAllPhases` (via `RavenDbPurePhase1Emulator` → the shared
`ScheduleTestPhase1EmulatorBase`):
...
`RavenDbPurePhase2Emulator` (via the shared `DataRetentionPhase2EmulatorBase`) reuses the ~999 jobs
...
- `ravendb-standalone` — its container is stopped and restarted (same `ClusterId`/database) with
...
- `ravendb-dist-one` — no `DataRetentionTtl` at all; its purpose here is purely the
...
- `ravendb-dist-two` — restarted with the *same* config Phase1 used (no `DataRetentionTtl`, no
...
`RavenDbPurePhase3Emulator` (via the shared `DataRetentionPhase3EmulatorBase`) only touches
`ravendb-dist-one` — the cluster whose buckets Phase2 already proved are fully drained and
