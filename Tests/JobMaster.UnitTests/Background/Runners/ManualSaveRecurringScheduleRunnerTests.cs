...
/// <summary>
/// Unit tests for <see cref="ManualSaveRecurringScheduleRunner"/>.
...
/// </summary>
public class ManualSaveRecurringScheduleRunnerTests
{
    private const string TestBucketId = "test-bucket";

    private static ManualSaveRecurringScheduleRunner CreateRunner(
        RunnerFixture f, string? bucketId = TestBucketId)
    {
        var runner = new ManualSaveRecurringScheduleRunner(f.Worker.Object);
        if (bucketId != null)
    ...
    public async Task OnTickAsync_WhenBucketIdNotDefined_ShouldReturnSkipped()
    {
        ...
        var runner = new ManualSaveRecurringScheduleRunner(f.Worker.Object); // no DefineBucketId
