...
// JSON-config counterpart to ConfigExtensions.UsePostgresForMaster/UsePostgresForAgent's tablePrefix
...
internal sealed class PostgresConnectionOptionsBinder : IConnectionOptionsBinder
{
    ...
    private static void ValidateKeys(IDictionary<string, object> options)
    {
        foreach (var key in options.Keys)
        {
            ...
            {
                throw new ArgumentException($"Unknown Postgres connection option '{key}'. Supported: {string.Join(", ", KnownKeys)}.");
            }
        }
    }
