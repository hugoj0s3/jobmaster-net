-- Vendored from https://github.com/quartznet/quartznet/blob/main/database/tables/tables_sqlServer.sql
-- (not distributed in the Quartz.NET NuGet package). Original "USE [enter_db_name_here]; GO" header
-- removed -- QuartzSchemaProvisioner opens the ADO.NET connection already pointed at the target
-- database via the connection string, so an explicit USE isn't needed (and would fail against the
-- placeholder name if left in). This script is idempotent (every DROP is IF OBJECT_ID(...) IS NOT
-- NULL-guarded), safe to run against a fresh or already-provisioned database.
