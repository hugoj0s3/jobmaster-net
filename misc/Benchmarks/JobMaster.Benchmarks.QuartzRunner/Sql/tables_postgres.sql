-- Vendored from https://github.com/quartznet/quartznet/blob/main/database/tables/tables_postgres.sql
-- (not distributed in the Quartz.NET NuGet package). Idempotent: the DO $$ ... END $$; block
-- guards every DROP TABLE with IF EXISTS, safe to run against a fresh or already-provisioned
-- database. Executed as a single multi-statement script (see QuartzSchemaProvisioner) --
-- Npgsql's simple query protocol parses the whole batch server-side, dollar-quoting included,
-- so splitting on ';' the way the SQL Server script is split on 'GO' would incorrectly break
-- this DO block apart.
