...
# JobMaster.MySql
### MySQL storage provider for JobMaster .Net.

This package provides the MySQL implementation for the **JobMaster .Net** engine, supporting both the Master (Coordination) and Agent (Transport) layers.
...
To use MySQL as your storage backend, register it during the cluster configuration in your `Program.cs`.
...
builder.Services.AddJobMasterCluster(config =>
{
    config.ClusterId("Production-Cluster")
          .UseMySqlForMaster("Your_Connection_String");
});
```
...
.UseMySqlForAgent("Your_Connection_String");
