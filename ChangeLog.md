...
- **`IJobMasterScheduler.Advanced` — schedule a job without referencing the handler's assembly** — For scenarios where the publisher and the consumer/worker are meant to be fully separate deployables, `Advanced.OnceNow`/`OnceAt`/`OnceAfter`/`Recurring` (+ `*Async`) let you schedule against a `JobDefinitionConfig` (an id plus optional priority/timeout/retries/worker-lane/metadata) instead of a concrete handler type. Define a `JobDefinitionConfigAttribute` subclass carrying that config in a shared "contracts" project — a `public static JobDefinitionConfig Config { get; }` member, with `IStaticJobDefinitionConfig` implemented to enforce it's declared (compile-time on the primary `net8.0` target, and validated again at cluster startup either way, so a subclass missing `Config` is always caught before it can cause a confusing failure later) — and the publisher references only that (`Advanced.OnceNow<TDefinition>(...)`, or build a `JobDefinitionConfig` inline with `Advanced.OnceNow(config, ...)`), while the consumer applies the same attribute to its `IJobMasterHandler` implementation. `Config` is deliberately static rather than per-instance: it's a fixed identity shared by every call site using that attribute type, so a `TDefinition` subclass can't be constructed differently to silently produce a different config for the same type. `JobDefinitionConfig` itself is not sealed, so you can derive your own subclass to carry extra fields if needed. The classic `OnceNow<THandler>()`/`Recurring<THandler>()` direct-handler methods are unchanged. Recurring schedules created this way still defer Priority/Timeout/MaxNumberOfRetries/WorkerLane resolution to each occurrence (same as classic recurring schedules always have), so a long-lived schedule picks up a definition's config changes on its next firing rather than freezing them in at creation. `IJobMasterSchedulerAdvanced` can also be injected directly from DI, if you only need the `Advanced` surface. ⚠️ This adds a new `Advanced` member to the public `IJobMasterScheduler` interface — if you have your own hand-written class implementing `IJobMasterScheduler` directly (e.g. a test double, not a mocking-framework proxy like `Mock<IJobMasterScheduler>`), it will need an `Advanced` implementation to keep compiling.

- **`[NaturalCronSchedule("...")]`/`[TimeSpanIntervalSchedule("...")]` — static recurring schedules without a profile class** — A lighter alternative to `IStaticRecurringSchedulesProfile` for the common case of one handler with one fixed schedule: decorate the handler class directly, e.g. `[NaturalCronSchedule("every 6 minutes")]` on an `IJobMasterHandler`, and it's registered automatically at cluster startup — no profile class needed. A handler can carry more than one such attribute if it needs more than one schedule. Registers to the default cluster; priority/timeout/retries/worker-lane come from the handler's own `[JobMasterPriority]`/`[JobMasterTimeout]`/etc. (or `JobDefinitionConfigAttribute`) if set, same as every other handler-level attribute. Both built-in recurrence compilers (`NaturalCron`, `TimeSpanInterval`) have an attribute; a custom compiler registered via `RecurrenceCompilerFactory.RegisterCompiler` can supply its own `RecurringScheduleAttribute` subclass the same way. `IStaticRecurringSchedulesProfile.Configure`'s `RecurringScheduleDefinitionCollection` also gained non-generic `Add(Type handlerType, ...)` overloads alongside the existing generic `Add<Th>(...)` ones, for the rarer case where you only have a handler `Type` at hand (e.g. your own reflection-driven registration) rather than a compile-time type parameter.

- **New RavenDB connection options: `requestTimeout`, `pooledConnectionLifetime`, `pooledConnectionIdleTimeout`** — Three new optional parameters on `UseRavenDb` (both the master/cluster and agent overloads), and their `ConfigFromJson` equivalents (`requestTimeoutMs`/`pooledConnectionLifetimeMs`/`pooledConnectionIdleTimeoutMs`, integer milliseconds). Useful for tuning RavenDB's HTTP behavior under load — in particular, `pooledConnectionLifetime`/`pooledConnectionIdleTimeout` control how long a pooled connection is kept before being recycled, which can help avoid client-side "Connection reset by peer" errors caused by a stale pooled connection outliving the server's own idle timeout. `pooledConnectionLifetime`/`pooledConnectionIdleTimeout` have no effect when running on `netstandard2.0` (silently ignored there — the underlying `SocketsHttpHandler` APIs they rely on aren't available on that target framework); `requestTimeout` works on both.
...
- **A worker's default `ParallelismFactor` now scales with its container's CPU instead of a fixed 1.0** — When `IAgentWorkerSelector.ParallelismFactor(...)` isn't called, a worker now defaults to its own `Environment.ProcessorCount` (capped at 10) rather than a flat multiplier of 1.0. This means a worker on a beefier container gets proportionally more concurrent execution capacity out of the box, and one on a small/constrained container gets less, without needing to set it explicitly. Explicitly calling `ParallelismFactor(...)` (or setting it via `ConfigFromJson`) is unaffected and always takes precedence over this default.

- **Per-priority base execution capacity changed from an arithmetic to a Fibonacci progression** — The base number of concurrent execution slots per bucket, per priority level, is now `VeryLow=2, Low=3, Medium=5, High=8, Critical=13` (was `2, 3, 4, 5, 6`). The old progression grew by a fixed +1 each step, so the relative gap between priority levels shrank the higher you went (Critical was only 3x VeryLow); the new one keeps a roughly constant ~1.6x ratio between adjacent tiers, so priority keeps mattering at the top end (Critical is now ~6.5x VeryLow). This multiplies with `ParallelismFactor` (see above), so the combined effect can be significant on a high-CPU worker — e.g. a 10-CPU worker's default Critical-priority capacity goes from 60 to 130 concurrent slots per bucket (and its waiting-queue capacity, `RunCapacity × 5`, from 300 to 650). If you're relying on the previous concurrency ceiling for Critical-priority work, consider setting `ParallelismFactor` explicitly rather than depending on the default.

### Fixed

- ⚠️ **Startup validation errors (duplicate `JobDefinitionId`s, workers missing `AgentConnectionName`, and similar) now actually stop startup** — Previously these checks ran and produced real error messages, but the result was silently discarded, so a misconfigured cluster would start anyway. If your cluster has one of these pre-existing issues, upgrading will surface it as a startup failure for the first time — fix the reported issue rather than treating this as a regression. A handler decorated with both a `JobDefinitionConfigAttribute` (see Added, above) and any classic individual attribute (`JobMasterDefinitionId`/`JobMasterTimeout`/`JobMasterPriority`/`JobMasterWorkerLane`/`JobMasterMaxNumberOfRetries`) is now also rejected at startup, since mixing the two usually means a stale attribute left over from switching modes. Coordinator-mode workers (which correctly have no `AgentConnectionName`) are exempted from the missing-`AgentConnectionName` check — found and fixed as part of wiring this up, so a cluster with a Coordinator worker won't be incorrectly rejected.

- **Retrying a failed job with attempts remaining could throw instead of retrying** — Recording a job's failed execution attempt while it still has retries left leaves the job's status `OnMaster` (a normal in-progress retry state), not `Failed` — but the update path only accepted a failed execution outcome when the job's own status was also `Failed`, so persisting that attempt-and-retry always threw an exception. Fixed by simplifying the check to only cross-validate the two statuses where job status and execution outcome are guaranteed to correspond: a `Succeeded` job requires a `Succeeded` outcome, and a `Failed` job requires a `Failed` outcome. Every other status (`OnMaster`, `InBucket`, etc.) now accepts any execution outcome, since only those two statuses have a fixed 1:1 relationship with an outcome in the first place.
