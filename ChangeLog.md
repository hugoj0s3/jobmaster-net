...
- **`[NaturalCronSchedule("...")]`/`[TimeSpanIntervalSchedule("...")]` — static recurring schedules without a profile class** — A lighter alternative to `IStaticRecurringSchedulesProfile` for the common case of one handler with one fixed schedule: decorate the handler class directly, e.g. `[NaturalCronSchedule("every 6 minutes")]` on an `IJobMasterHandler`, and it's registered automatically at cluster startup — no profile class needed. A handler can carry more than one such attribute if it needs more than one schedule. Registers to the default cluster; priority/timeout/retries/worker-lane come from the handler's own `[JobMasterPriority]`/`[JobMasterTimeout]`/etc. (or `JobDefinitionConfigAttribute`) if set, same as every other handler-level attribute. Both built-in recurrence compilers (`NaturalCron`, `TimeSpanInterval`) have an attribute; a custom compiler registered via `RecurrenceCompilerFactory.RegisterCompiler` can supply its own `RecurringScheduleAttribute` subclass the same way.
