...
// JSON-config counterpart to ConfigExtensions.UseSqlServerForMaster/UseSqlServerForAgent's tablePrefix
...
internal sealed class SqlServerConnectionOptionsBinder : IConnectionOptionsBinder
{
    ...
    private static void ValidateKeys(IDictionary<string, object> options)
    {
        foreach (var key in options.Keys)
        {
            ...
            {
                throw new ArgumentException($"Unknown SQL Server connection option '{key}'. Supported: {string.Join(", ", KnownKeys)}.");
            }
        }
    }
