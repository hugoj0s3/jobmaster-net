...
One cluster (`postgres-drain-load`), one master database, **3 agent connections**
(`pg-agent-1/2/3`, each its own agent-side database) — one per worker throughout, matching how
...
|---|---|---|
| 1 | `pg-coordinator`, `pg-executor-1`, `pg-executor-2`, `pg-executor-3` | Coordinator, Execution ×3 |
| 2 | `pg-coordinator`, `pg-drainer-1`, `pg-drainer-2`, `pg-drainer-3` | Coordinator, Drain ×3 |
| 3 | `pg-coordinator`, `pg-executor-1`, `pg-executor-2`, `pg-executor-3` | Coordinator, Execution ×3 |

Each connection keeps the same worker identity throughout: `pg-executor-N` (Phase1) dies, is drained
by `pg-drainer-N` (Phase2, same `AgentConnectionName: pg-agent-N`), then `pg-executor-N` (Phase3,
container name reused — safe, since it was already explicitly stopped in Phase1 before Phase2 ever
starts) returns on the exact same connection. This mirrors a realistic rolling
crash/recycle-then-restore per connection, not an arbitrary pool sharing one connection with no real
reason to need 3 workers.

`pg-coordinator` is re-declared unchanged in every phase for explicitness.
...
stopped *explicitly*, via `Runner.StopAsync(...)` at the end of `PostgresDistPhase1Emulator`, not by
...
`PostgresDistPhase2Emulator` doesn't fight this, and — discovered the hard way after a couple of
local runs — doesn't wait for it either. The fallback bucket lives on its own separate reserved
connection (`JobMasterConstants.MasterFallbackAgentConnName`), entirely independent of
`pg-agent-1/2/3`, and is served by exactly one serial runner — at 5000-job volume that trickle simply
cannot finish within any timeout reasonable for this phase, and it doesn't need to: finishing
...
this plan, shared by all 3 phases — each phase is a separate `Activator.CreateInstance` instance with
no in-memory hand-off from the one before it, so every phase recomputes the same deterministic
...
- **Phase1**: schedules all 5000 jobs (via `pg-executor-1`'s HTTP endpoint — which container issues
...
Building this scenario surfaced a genuine bug, not just a test-design lesson: `MarkBucketAsLostRunner`
(the periodic sweep that detects a dead worker's orphaned buckets) called
...
A full passing run (5000 jobs, 3 phases, crashing immediately post-schedule so jobs are caught across
every pipeline stage — see Phase1 above) took **1h4m** with Phase3 waiting for full bucket destruction
and widened `FinalizeTimeout` (45 min, to absorb real contention between the Coordinator's shared
runner semaphore serving new-job onboarding and still-active old-bucket cleanup concurrently — Phase2
no longer waits for the old buckets to fully drain before Phase3 starts). Phase2 itself is fast
(~1-2 min) since it only waits for durable save, not bucket drain. Relaxing Phase3's bucket check from
"destroyed" to "reached `ReadyToDelete`" (`OldBucketsResolvedTimeout` 60 → 20 min) should meaningfully
shorten the tail without losing signal — most of that window elapses in parallel with the
`FinalizeTimeout` wait anyway. Re-tighten or loosen the timeouts if a real run's timing drifts
materially from what's encoded here.
