...
/// workers, each on the *same* agent connection its dead counterpart used (drainer-1/sqlserver-agent-1,
/// drainer-2/sqlserver-agent-2, drainer-3/sqlserver-agent-3) -- AssignedLostBucketsRunner reassigns a dead
...
/// PollingDrainProcessingJobsRunner (already-onboarded jobs).
///
...
/// </summary>
public sealed class SqlServerDistPhase2Emulator(ScenarioGlobalEnvironment global, ScenarioRunner runner)
    : BasePhaseEmulator<SqlServerDistPhases>(global, runner)
{
    private const string ClusterId = "sqlserver-drain-load";
    private static readonly string[] AgentConnectionNames = ["sqlserver-agent-1", "sqlserver-agent-2", "sqlserver-agent-3"];
    ...
    public override SqlServerDistPhases Phase() => SqlServerDistPhases.Phase2;
