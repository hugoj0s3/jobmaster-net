...
/// </summary>
public sealed class SqlServerDistPhase1Emulator(ScenarioGlobalEnvironment global, ScenarioRunner runner)
    : BasePhaseEmulator<SqlServerDistPhases>(global, runner)
{
    private const string ClusterId = "sqlserver-drain-load";
    private static readonly string[] ExecutorContainers = ["sqlserver-executor-1", "sqlserver-executor-2", "sqlserver-executor-3"];

    public override SqlServerDistPhases Phase() => SqlServerDistPhases.Phase1;

    public override async Task RunAsync()
    {
        foreach (var batch in DrainModeTestPlan.Batches)
        {
            await Runner.ScheduleFor("sqlserver-executor-1")
