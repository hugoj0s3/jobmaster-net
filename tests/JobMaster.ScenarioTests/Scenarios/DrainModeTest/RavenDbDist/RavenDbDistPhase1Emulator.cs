...
/// </summary>
public sealed class RavenDbDistPhase1Emulator(ScenarioGlobalEnvironment global, ScenarioRunner runner)
    : BasePhaseEmulator<RavenDbDistPhases>(global, runner)
{
    private const string ClusterId = "ravendb-drain-load";
    private static readonly string[] ExecutorContainers = ["ravendb-executor-1", "ravendb-executor-2", "ravendb-executor-3"];

    public override RavenDbDistPhases Phase() => RavenDbDistPhases.Phase1;

    public override async Task RunAsync()
    {
        foreach (var batch in DrainModeTestPlan.Batches)
        {
            await Runner.ScheduleFor("ravendb-executor-1")
